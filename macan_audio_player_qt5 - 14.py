...
from PyQt5.QtGui import QIcon, QPainter, QColor, QPixmap, QCursor, QTextCursor, QFont, QTextCharFormat, QTextBlockFormat, QImage
...
SVG_ICONS = {
    ...
    "album-placeholder": "",
...
class AudioPlayer(QMainWindow):
    ...
    def __init__(self):
        ...
        # [MODIFIKASI] Ukuran default diatur di sini, tetapi posisi akan ditengahkan
        self.resize(850, 650)
        ...
        # [MODIFIKASI] Panggil fungsi untuk menengahkan jendela
    ...
    # [BARU] Fungsi untuk menempatkan jendela di tengah layar
    def center_on_screen(self):
        """Pusatkan jendela di tengah layar yang tersedia."""
        # Dapatkan geometri frame jendela
        frame_geometry = self.frameGeometry()
        # Dapatkan geometri layar yang tersedia
        screen_center = QApplication.primaryScreen().availableGeometry().center()
        # Pindahkan titik tengah frame ke titik tengah layar
    ...
    def create_title_bar(self):
        ...
        # [BARU] Tambahkan Aksi Library di sini
        ...
        # [FITUR BARU] Tombol toggle Dynamic Aura
    ...
    def load_settings(self):
        ...
        # [FITUR BARU] Muat status Dynamic Aura
    ...
    # Di dalam class AudioPlayer di file macan_audio_player_qt5 - 12.py
    ...
    def closeEvent(self, event):
        ...
        # [FITUR BARU] Simpan status Dynamic Aura
        ...
        # [FIX] Panggil save_data dari library secara eksplisit saat aplikasi ditutup
        if self.library_window:
            self.library_window.save_data()
        
        # [BARU] Pastikan mini player & tray icon juga ditutup
        self.mini_player.close()
        if self.radio_window:
            self.radio_window.close()
        self.tray_icon.hide()
        
        # [FIX] Memanggil QApplication.instance().quit() secara eksplisit
        # untuk memastikan event loop Qt berhenti dan aplikasi tertutup sepenuhnya.
        # Ini diperlukan karena QApplication.setQuitOnLastWindowClosed(False)
        # diatur agar fitur "minimize to tray" berfungsi dengan benar.
    ...
    def clear_playlist(self):
        ...
        # [FITUR BARU] Reset Dynamic Aura color and re-apply base theme
        self.current_dynamic_color = None
        self.apply_current_visuals()
    ...
    def update_track_info(self, track_path):
        ...
        # [FITUR BARU] Update dynamic color if enabled
        if self.dynamic_aura_enabled:
        ...
        # [FITUR BARU] Apply the visual update
        self.apply_current_visuals()
    ...
    def get_track_metadata(self, path):
        ...
        try:
            ...
            # [FIX] Changed M4A(path) to MP4(path) for better compatibility
    ...
    # --- [FITUR BARU] Logika Dynamic Aura ---
    def toggle_dynamic_aura(self, enabled):
        """Mengaktifkan atau menonaktifkan fitur Dynamic Aura."""
        self.dynamic_aura_enabled = enabled
        if enabled and self.current_index != -1:
            # Jika diaktifkan saat lagu diputar, ekstrak warna dari lagu saat ini
            path = self.playlist[self.current_index]
            metadata = self.get_track_metadata(path)
            artwork = metadata.get('artwork')
            self.current_dynamic_color = self.extract_dominant_color(artwork)
        else:
            # Jika dinonaktifkan, hapus warna dinamis
            self.current_dynamic_color = None
        self.apply_current_visuals()

    def extract_dominant_color(self, artwork_data):
        """Mengekstrak warna dominan yang cocok dari data gambar album art."""
        if not artwork_data:
            return None
        ...
        # Perkecil gambar menjadi 1x1 piksel untuk mendapatkan warna rata-rata
        image = pixmap.toImage().scaled(1, 1, Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
        color = QColor(image.pixel(0, 0))

        # Pastikan warna tidak terlalu gelap, terlalu terang, atau pudar
    ...
    def apply_current_visuals(self):
        ...
        if self.dynamic_aura_enabled and self.current_dynamic_color:
            ...
            # Jika tidak, gunakan tema dasar
            self.apply_stylesheet(base_theme)

    # [BARU] Fungsi untuk membuka dan mengelola jendela library
    def show_library_window(self):
        """Membuat (jika belum ada) dan menampilkan jendela library."""
        if self.library_window is None:
            ...
            # Hubungkan sinyal dari library ke slot di player utama
    ...
    def show_radio_window(self):
        ...
        # Hubungkan sinyal agar player utama berhenti saat radio diputar
    ...
    # [BARU] Slot untuk menerima data dari library
    def handle_library_selection(self, data):
        """Menangani lagu yang dikirim dari library."""
        ...
        if action == 'play':
            # Hapus playlist saat ini dan mainkan lagu baru dari library
            self.clear_playlist()
            self.add_and_play_paths(paths)
        elif action == 'append':
            # Tambahkan lagu ke akhir playlist yang ada
            new_tracks = []
            for path in paths:
            ...
            if new_tracks:
                ...
                self.update_playlist_display()
                self.update_status_bar()
                QMessageBox.information(self, "Lagu Ditambahkan", f"{len(new_tracks)} lagu telah ditambahkan ke antrian.")

    def apply_stylesheet(self, theme):
        """Menerapkan QSS dan properti UI berdasarkan dictionary tema yang diberikan."""
        self.visualizer_widget.set_accent_color(theme["accent"])
        self.mini_player.apply_stylesheet(theme)

        # [BARU] Tambahkan blok if ini untuk update tema library
        if self.library_window:
            self.library_window.apply_stylesheet(theme)

        # [BARU] Tambahkan blok if ini untuk update tema radio
        if self.radio_window:
            self.radio_window.apply_stylesheet(theme)
