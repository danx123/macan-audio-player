# Nama file: macan_audio_player_revised_frameless.py
# Deskripsi: Versi revisi dengan desain frameless, kontrol jendela kustom, dan layout yang disesuaikan.
# Jalankan dengan perintah: python macan_audio_player_revised_frameless.py
#
# Pastikan pustaka yang dibutuhkan sudah terinstal:
# pip install PyQt6 PyQt6-QtMultimedia mutagen
#
# Perubahan dari versi sebelumnya:
# 1. [FEATURE] Jendela dibuat frameless (tanpa title bar standar).
# 2. [FEATURE] Menambahkan kontrol jendela kustom (minimize, maximize/restore, close) dengan ikon SVG.
# 3. [FEATURE] Mengimplementasikan fungsionalitas untuk memindahkan jendela dengan mouse (drag).
# 4. [UI/UX] Ukuran album art diperkecil agar tidak terlalu dominan.
# 5. [UI/UX] Area playlist dibuat lebih tinggi/besar untuk visibilitas yang lebih baik.
# 6. [REFACTOR] Kode pembuatan UI diatur ulang untuk mengakomodasi title bar kustom.
...
from PyQt6.QtCore import Qt, QUrl, QTimer, pyqtSignal, QSize, QSettings, QPoint
...
# --- [AESTHETICS] DATA SVG IKON BARU (TERMASUK KONTROL JENDELA) ---
SVG_ICONS = {
...
class AudioPlayer(QMainWindow):
    def __init__(self):
        super().__init__()
        # [FEATURE] Membuat jendela frameless
    ...
    def setup_ui(self):
        # Wrapper utama dengan border radius untuk estetika frameless
        ...
        # [FEATURE] Title bar kustom
        main_layout.addLayout(self.create_title_bar())
        
        # Wrapper untuk konten utama dengan margin
        content_layout = QVBoxLayout()
        content_layout.setContentsMargins(20, 0, 20, 0)
        content_layout.setSpacing(15)

        content_layout.addLayout(self.create_info_panel())
        content_layout.addLayout(self.create_seekbar_panel())
        content_layout.addLayout(self.create_playback_controls())
        content_layout.addLayout(self.create_bottom_panel())
        
        self.playlist_widget = QListWidget()
        self.playlist_widget.setObjectName("playlistWidget")
        # [UI/UX] Stretch factor lebih besar untuk playlist
    ...
    def create_title_bar(self):
        ...
        # [FEATURE] Area untuk drag window
        self.title_label = QLabel("Macan Audio Player 🎵")
        self.title_label.setObjectName("titleLabel")
        
        # Menu Opsi
        self.options_button = QPushButton()
        self.options_button.setIcon(self._create_button("add-folder", "Opsi").icon()) # Placeholder icon
        ...
        # [FEATURE] Tombol kontrol jendela kustom
        ...
        self.close_button.setObjectName("closeButton") # ID khusus untuk styling merah
    ...
    def create_info_panel(self):
        ...
        # [UI/UX] Ukuran album art diperkecil
        self.album_art_label.setFixedSize(150, 150) 
    ...
    # [FEATURE] Fungsi untuk maximize/restore jendela
    ...
    # [FEATURE] Event handler untuk memindahkan jendela
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            # Cek jika kursor ada di area title bar
            ...
            for filename in sorted(os.listdir(folder_path)):
                if any(filename.lower().endswith(ext) for ext in supported_formats):
                    ...
                    if full_path not in self.playlist:
                        ...
                        new_tracks_count += 1
            if new_tracks_count > 0 and self.current_index == -1:
                self.current_index = 0
                self.playlist_widget.setCurrentRow(0)
                self.update_track_info(self.playlist[0])
    ...
    def apply_stylesheet(self):
        qss = """
            ...
            #titleLabel { font-weight: bold; padding: 10px; }
            ...
            #mainButton {
                background-color: #0d6efd; color: #FFFFFF; border-radius: 32px;
                min-width: 64px; min-height: 64px; padding: 8px;
            }
