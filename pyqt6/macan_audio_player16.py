...
# [MODIFIKASI] Menambahkan QTextCharFormat dan QTextBlockFormat untuk mengelola format teks lirik
from PyQt6.QtGui import QIcon, QPainter, QColor, QAction, QPixmap, QCursor, QTextCursor, QFont, QTextCharFormat, QTextBlockFormat
...
class AudioPlayer(QMainWindow):
    def __init__(self):
        ...
        # [BARU] Variabel untuk menyimpan format teks lirik (normal dan di-highlight)
        # Ini akan diatur warnanya di dalam apply_stylesheet
        self.normal_lyric_format = QTextCharFormat()
        self.highlighted_lyric_format = QTextCharFormat()
        
        # Variabel untuk auto-resume
    ...
    def create_title_bar(self):
        ...
        # --- [MODIFIKASI BARU] Menambahkan Submenu About dan Exit ---
        options_menu.addSeparator()
        ...
        exit_action.triggered.connect(self.close) # Menghubungkan ke fungsi close window
    ...
    # --- [MODIFIKASI BARU] Fungsi untuk menampilkan dialog About ---
    def show_about_dialog(self):
        ...
        <p>Ditenagai oleh PyQt6 dan FFmpeg, aplikasi ini mendukung berbagai format audio populer dengan performa ringan, cepat, dan stabil.</p>
    ...
    # [MODIFIKASI] Fungsi ini diubah total untuk memasukkan teks sekali saja
    def display_full_lyrics(self):
        self.lyrics_viewer.clear()
        if not self.lyrics_data: return

        # Menggunakan QTextCursor untuk memasukkan teks dengan format
        cursor = self.lyrics_viewer.textCursor()
        
        # Atur agar semua teks berada di tengah
        block_format = QTextBlockFormat()
        block_format.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cursor.setBlockFormat(block_format)

        # Jika lirik hanya teks biasa (bukan LRC), tampilkan seperti biasa
        if len(self.lyrics_data) == 1 and self.lyrics_data[0][0] == 0:
            cursor.insertText(self.lyrics_data[0][1], self.normal_lyric_format)
        # Jika lirik berformat LRC, masukkan baris per baris
        else:
            for _, text in self.lyrics_data:
                line_text = text if text else ""
                cursor.insertText(line_text + '\n', self.normal_lyric_format)
            # Hapus baris baru ekstra di akhir
            cursor.deletePreviousChar()
        
        # Pindahkan kursor ke awal dokumen
        self.lyrics_viewer.moveCursor(QTextCursor.MoveOperation.Start)

    # [MODIFIKASI] Fungsi ini diubah untuk hanya memanggil highlight_lyric_line
    def sync_lyrics(self, position):
        ...
        if found_index != self.current_lyric_index:
            # Panggil fungsi highlight dengan indeks lama dan baru
    ...
    # [MODIFIKASI] Fungsi ini diubah total untuk mengubah format teks, bukan menulis ulang HTML
    def highlight_lyric_line(self, old_line_index, new_line_index):
        doc = self.lyrics_viewer.document()

        # 1. Kembalikan format baris lama ke normal
        ...
        # 2. Terapkan format highlight ke baris baru
        if new_line_index >= 0 and new_line_index < doc.blockCount():
            ...
            # 3. Atur kursor ke baris yang di-highlight dan pastikan terlihat (auto-scroll)
    ...
    def apply_stylesheet(self):
        ...
        # [BARU] Atur warna untuk format lirik normal dan highlight sesuai tema
        self.normal_lyric_format.setForeground(QColor(theme["text_primary"]))
        self.normal_lyric_format.setFontWeight(QFont.Weight.Normal)
        self.highlighted_lyric_format.setForeground(QColor(theme["accent"]))
        self.highlighted_lyric_format.setFontWeight(QFont.Weight.Bold)
