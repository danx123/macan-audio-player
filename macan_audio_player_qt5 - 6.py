...
class VisualizerWidget(QWidget):
    def __init__(self, parent=None):        
        ...
        # [BARU] Tambahkan mode visualizer
        self.visualizer_mode = 0
        self.setToolTip("Klik untuk mengubah gaya visualizer")

    # [BARU] Tambahkan event klik mouse
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            self.visualizer_mode = (self.visualizer_mode + 1) % 3 # Ada 3 mode (0, 1, 2)
            self.update()
        super().mousePressEvent(event)
    ...
    def update_bars(self):
        ...
        for i in range(num_bars):
            ...
            # [MODIFIKASI] Sesuaikan tinggi berdasarkan mode
            if self.visualizer_mode == 1: # Mode bar dari bawah
                 self.bars[i] = int(base_height * (1 + eq_mod) * 2)
            else: # Mode default dan mode cermin
    ...
    def paintEvent(self, event):
        ...
        for i, bar_height in enumerate(self.bars):
            ...
            # [BARU] Logika rendering berdasarkan mode
            if self.visualizer_mode == 0: # Mode Default (Rounded Center)
                y = self.height() // 2 - height // 2
                painter.drawRoundedRect(int(x), int(y), int(bar_width - 2), int(height), 2, 2)
            
            elif self.visualizer_mode == 1: # Mode Bar dari Bawah
                y = self.height() - height
                painter.drawRect(int(x), int(y), int(bar_width - 2), int(height))

            elif self.visualizer_mode == 2: # Mode Cermin (Mirrored)
...
class AudioPlayer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # [BARU] Aktifkan context menu policy
    ...
    def create_playback_controls(self):
        ...
        ## [FIX 2 - GANTI UKURAN TOMBOL] Nama objek diganti menjadi "secondaryButton" agar ukurannya sesuai
    ...
    # [BARU] Menampilkan dialog properti file
    ...
    # [BARU] Membuka lokasi file di file explorer
    def open_file_location(self):
        ...
        try:
            if system == "Windows":
                # /select, akan membuka folder dan memilih file
                subprocess.Popen(f'explorer /select,"{path}"')
            elif system == "Darwin": # macOS
                # -R akan menampilkan file di Finder
