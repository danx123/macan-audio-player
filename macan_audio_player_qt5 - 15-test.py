...
SVG_ICONS = {
    ...
    # [BARU] Ikon untuk tombol toggle playlist
...
class AudioPlayer(QMainWindow):
    ...
    def open_file_location(self):
        ...
        try:
            if system == "Windows": subprocess.Popen(f'explorer /select,"{path}"')
            elif system == "Darwin": subprocess.Popen(["open", "-R", path])
            else:
    ...
    def closeEvent(self, event):
        ...
        self.settings.setValue("skin", self.current_skin) # [SKIN BARU] Simpan skin
    ...
    def save_playlist_to_file(self):
        if not self.playlist:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Simpan Playlist", self.last_opened_folder, "M3U Playlist (*.m3u)")
        if file_path:
            try:
    ...
    def load_playlist_from_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Buka Playlist", self.last_opened_folder, "M3U Playlist (*.m3u)")
        if file_path:
            try:
            ...
            except Exception as e: QMessageBox.critical(self, "Error", f"Gagal memuat playlist:\n{e}")
    ...
    def load_lyrics(self, track_path):
        ...
        if not track_path:
            self.lyrics_status_label.setText("Lirik tidak tersedia."); self.lyrics_status_label.show(); self.lyrics_viewer.hide()
            return
        ...
        if self.lyrics_data:
            ...
            self.lyrics_status_label.setText("Lirik tidak ditemukan."); self.lyrics_status_label.show(); self.lyrics_viewer.hide()
    ...
    def apply_skin_layout(self):
        ...
        if mode == "normal":
            ...
            # Kembalikan kemampuan resize
            self.setMinimumSize(self.skin_configs["normal"]["min_size"])
            self.setMaximumSize(16777215, 16777215)
            self.resize(self.sizeHint()) # Ukuran akan menyesuaikan konten
